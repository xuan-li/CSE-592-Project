...
def approximate_gradient_two_steps(
    func, x, current_state, last_state, direction_generator, t, m=1
):
    '''
    direction: direction vector
    t: smooth parameter
...
def stars(
    ...
    for k in range(maximum_iterations):
        ...
        gradient = approximate_gradient_two_steps(
            func, x, last_random_state, current_random_state,
            direction_generator, mu, m
        )
        x = x - h * gradient
        xs.append(x.copy())
        runtimes.append(time.time() - start_time)
